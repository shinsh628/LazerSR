...
## osu! 호스트 도입 (2026-09-21)

osu! 2026.920.0-lazer가 Release 빌드에서 startup hook을 껐다(`StartupHookSupport=false`, `architecture.md` §1).
**의도적인 변경이 맞지만 이유는 어디에도 적혀 있지 않다** — PR 없이 직접 push된 커밋이고, 치팅·모드 차단 같은
사유 언급이 커밋·PR·이슈·릴리즈 노트 어디에도 없다(조사 경위 `progress\2026-09-21.md`).

**사용자 결정(2026-09-21)**: 치팅 방지 등 명시된 이유가 있었다면 중단했겠지만, 이유가 없고 LazerSR에는 치팅 기능이
없으며(맵 난이도 분석 같은 유틸만), 지인 4명이 쓰는 비영리 토이 프로그램이므로 **그대로 진행**한다. 진입 수단만
커스텀 호스트로 바꾼다.

호스트가 레드라인에 미치는 영향 — **없다**:

- **레드라인 3(osu! 파일 쓰기 금지) 유지.** 호스트는 osu! 설치 폴더의 파일을 읽기만 한다. 바뀌는 건 이 프로세스
  메모리 안의 런타임 프로퍼티 하나(`System.StartupHookProvider.IsSupported`)뿐이다.
- **"환경변수는 자식 osu! 프로세스에만" 유지.** `DOTNET_STARTUP_HOOKS` 등은 여전히 런처가 띄우는 그 프로세스에만 설정된다.
- **레드라인 1·2 무관.** 호스트는 관리 코드가 뜨기 전에 끝나는 네이티브 부트스트랩이고 네트워크를 쓰지 않는다.
  그 뒤의 Hook 동작(패치·IPC·계산기)은 한 줄도 안 바뀌었다.
- 서버 제출값(점수·정확도·판정·리플레이)에 닿는 경로가 새로 생기지 않는다.

## 향후 리스크 대응

ppy가 hook 탐지 로직을 추가할 가능성은 낮지만 0은 아니다 (`skin-widget-research.md` §14 — 코드 한 줄로 추가 가능하다고 평가됨).
**중단 기준(2026-09-21 재정의)**: ppy가 **치팅·외부 수정 방지 같은 이유를 명시**해 막거나, 탐지·제재 조치가 확인되면:
1. 즉시 사용 중단 안내
2. 런처가 호스트/`DOTNET_STARTUP_HOOKS` 없이 osu!를 띄우도록(또는 실행 자체를 막도록) 긴급 패치

이유 없는 기술적 차단(2026.920.0의 startup hook 비활성화 같은 것)은 이 기준에 해당하지 않는다 — 위 "osu! 호스트 도입" 참고.
