...
| `LazerSR.Hook\HookRuntimeContext.cs` | 여러 패치가 얻은 RealmAccess/Storage 공유 홀더. `architecture.md` §22 | **관리대상** (2026-08-31 신규) |
| `LazerSR.Hook\ReplayUpload\` | 리플레이 저장 서버로 보낼 큐 작성(realm 스캔, 네트워크 없음). `architecture.md` §22 | **관리대상** (2026-08-31 신규) |
...
| `LazerSR.Launcher\Upload\` | 리플레이 저장 서버 업로더 - 큐 폴링 + 자동 등록 (`architecture.md` §22) | **관리대상** (2026-08-31 신규) |
